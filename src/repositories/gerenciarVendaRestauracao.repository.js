			...
			connection.query(sqlSelecionarIdCliente, [dados.inputCPF], function (err, resultCliente) {
				...
				connection.query(sqlSelecionarIdEmpresa, [dados.inputCompanyState], function (err, resultEmpresa) {
					...
					if (dados.ehVenda) {
						...
						connection.query(sqlSelecionarQuantAtual, [dados.inputId, idEmpresa], function (err, resultQuantAtual) {
							if (err) {
								console.log("Erro ao selecionar a quantidade atual do estoque no banco de dados: ", err);
								reject(err);
							}
							...
							// Atualizar o estoque
							connection.query(sqlAtualizarEstoque, [dados.inputQuantity, dados.inputId, idEmpresa], function (err, resultAtualizacaoEstoque) {
								if (err) {
									console.log("Erro ao atualizar o estoque no banco de dados: ", err);
									reject(err);
								}
							...
							if (quantAtual - dados.inputQuantity === 0) {
								...
								connection.query(sqlDeletarEstoque, [dados.inputId, idEmpresa], function (err, resultDeletarEstoque) {
									if (err) {
										console.log("Erro ao deletar o estoque no banco de dados: ", err);
										reject(err);
									}
